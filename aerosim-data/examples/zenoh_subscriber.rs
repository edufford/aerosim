...
use aerosim_data::middleware::{
    Metadata, Middleware, MiddlewareRaw, MiddlewareRegistry, Serializer,
};
...
async fn subscribe_to_vector3(registry: &MiddlewareRegistry) -> Result<(), Box<dyn Error>> {
    ...
    middleware
        .subscribe(
            "vector3",
            Box::new(|stamp: Vector3, _metadata: Metadata| {
                println!("Received vector: {:?}", stamp);
                Ok(())
            }),
        )
        .await
...
async fn subscribe_raw(registry: &MiddlewareRegistry) -> Result<(), Box<dyn Error>> {
    ...
    middleware
        .subscribe_all_raw(
            vec![
                ("aerosim::types::JsonData".to_string(), "json".to_string()),
                ("aerosim::types::Vector3".to_string(), "vector3".to_string()),
            ],
            Box::new(move |payload: &[u8]| {
                ...
                match metadata.type_name.as_str() {
                    "aerosim::types::JsonData" => println!(
                        "Received raw json data: {:?}",
                        serializer.deserialize_data::<JsonData>(payload)
                    ),
                    "aerosim::types::Vector3" => println!(
                        "Received raw vector: {:?}",
                        serializer.deserialize_data::<Vector3>(payload)
                    ),
